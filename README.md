...
`pruneNotification` controls the detail level. Automatic compression notices are always shown immediately as TUI notifications, including when `pruneNotificationType` is `chat`. OMP cannot append a chat message during an active tool run without either hiding it until the run ends or steering the agent and skipping remaining tools. `pruneNotificationType` therefore applies to automatic tool-pruning notices and manual command feedback: pruning is aggregated across the agent run and emitted once when the run finishes, while manual feedback remains immediate. Detailed notices show cumulative and per-operation token removal, pruning reasons, affected tool names and identifying arguments, plus compression run metrics, topics, message/tool counts, summary-token cost, and a context-history progress bar. Minimal notices remain single-line summaries. `compress.showCompression` adds the generated summary text to the compression notice; it does not suppress the base compression metrics. Toast bodies are bounded to keep the TUI usable.
